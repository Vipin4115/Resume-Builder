...
const PersonalInfoForm = ({data,onChange,removeBackground,setRemoveBackground}) => {
  ...
  return (
    <div>
        ...
        <p className='text-sm text0gray-600'>Get Started with the personal information</p>
        <div className='flex items-center gap-2'>
            <label>
                {
                    data.image ? (
                        <img src={typeof data.image === 'string'? data.image : URL.createObjectURL(data.image)} 
                        alt="user-image" className='w1-16 h-16 rounded-full object-cover mt-5
                ...
                return(
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        <input type={field.type} value={data[field.key] || ""} onChange={(e)=>
                            handelChange(field.key,e.target.value)
