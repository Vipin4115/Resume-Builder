...
const MinimalImageTemplate = ({ data = {}, accentColor = "#3B82F6" }) => {
  ...
  // If personal_info.image is a File object, create an object URL safely and revoke it on cleanup.
  const imageFile = data.personal_info?.image && typeof data.personal_info.image === "object"
    ? data.personal_info.image
    : null;

  const objectUrl = useMemo(() => {
    return imageFile instanceof File ? URL.createObjectURL(imageFile) : null;
  }, [imageFile]);
            ...
            <section>
              ...
              <ul className="space-y-1 text-sm">
                {skills.map((skill, idx) => (
                  <li key={(skill && skill._id) ?? skill ?? idx}>
              ...
              <div className="space-y-4">
                {projects.map((project, idx) => (
                  <div key={project._id ?? idx}>
                    ...
                    {project.type && <p className="text-sm mb-1" style={{ color: accentColor }}>{project.type}</p>}
                    {project.description && (
